...
import { ApiSign } from './FCoinSign';
import * as crypto from 'crypto';
import { CodeObj, Code } from './Code';
...
export namespace FMex {
  export class Api {
    ...
    // 资产查询
    async FetchBalance(ks: KeySecret) {
      // [可用余额, 订单冻结金额, 仓位保证金金额]
      return this.api.get(`/v3/contracts/accounts`, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } }).then((res) => res.data as CodeObj<{ [index: string]: number[] }>);
    }

    async CancelOrders(ks: KeySecret, symbol: string) {
      return this.api
        .post(`/v3/contracts/orders/cancel`, { symbol }, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } })
        .then((res) => res.data as CodeObj<{ cancelled: number }>);
    }

    // 仓位
    async FetchPosition(ks: KeySecret) {
      return this.api.get(`/v3/broker/auth/contracts/positions`, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } }).then(
        (res) =>
          res.data as CodeObj<{
            results: {
              direction: 'short' | 'long'; // #仓位方向
              updated_at: number; // #仓位最近修改的毫秒时间戳
              quantity: number; // #持有合约数量，必须为正，若为0，表示已全部平仓
              leverage: number; // #仓位设定杠杆率，不一定等于实际杠杆率
              margin_leverage: number; // #实际杠杆率
              riskLevel: number; // #风险等级
              max_quantity: number; // #持有合约数量最大值
              realized_pnl: number; // #已实现盈亏，已实现盈亏仅作为统计展示给用户，表示自开仓以来该仓位的盈亏
              unrealized_pnl: number; // #未实现盈亏
              taker_fee_rate: number; // #开仓时确定的仓位平仓taker费率
              margin: number; // #仓位保证金，必须为正
              bankruptcy_price: number; // #破产价格，以该价格平仓，扣除taker手续费后，其权益恰好为0
              liquidation_price: number; // #强平价格，以该价格平仓，扣除taker手续费后，其剩余权益恰好为仓位价值 x 维持保证金率
              entry_price: 11017.0; // #开仓均价，每次仓位增加或减少时，开仓均价都会调整
              symbol: 'BTCUSD_P'; // #仓位合约代码，例如BTCUSD_P
              closed: boolean; // #仓位是否关闭
              minimum_maintenance_margin_rate: number; // #最小维持保证金，如果仓位保证金降低到此，将立刻触发强平
            }[];
          }>
      );
    }
    ...
    // 查询订单
    async Orders(ks: KeySecret) {
      return this.api.get(`/v3/contracts/orders/open`, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } }).then(
        (res) =>
          res.data as CodeObj<{
            results: OrderDto[];
          }>
    ...
    // 查询订单
    async OrderById(ks: KeySecret, id: string | number) {
      return this.api.get(`/v3/contracts/orders/${id}`, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } }).then((res) => res.data as CodeObj<OrderDto>);
    }

    async OrderCancel(ks: KeySecret, id: string | number) {
      return this.api.post(`/v3/contracts/orders/${id}/cancel`, null, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } }).then((res) => res.data as CodeObj<any>);
    }

    async SetLeverage(ks: KeySecret, symbol: string, leverage: number) {
      return this.api.post(`/v3/contracts/positions/${symbol}/leverage`, { leverage }, { headers: { 'FC-ACCESS-KEY': ks.Key, 'FC-ACCESS-SECRET': ks.Secret } }).then((res) => res.data as CodeObj<any>);
    }
    ...
    /** */
    async GetTicker(CoinSymbol: CoinSymbol) {
      return this.api.get(`/v2/market/ticker/${CoinSymbol}`).then((res) => res.data as CodeObj<WsTickerRes>);
    }
  ...
  export class Ws {
    ...
    private event = new EventEmitter(); // 使用 Vue ，作为事件管理者
    ...
    private wssConn() {
      if (this.key && this.secret) {
        ...
        const secret = `GEThttps://api.fmex.com/v2/user/ws${time}`;
        const signtmp = crypto.createHmac('sha1', this.secret).update(new Buffer(secret).toString('base64')).digest().toString('base64');
        const url = `wss://api.fmex.com/v2/user/ws?FC-ACCESS-KEY=${this.key}&FC-ACCESS-TIMESTAMP=${time}&FC-ACCESS-SIGNATURE=${signtmp}`;
        this.baseUrl = url;
      }
    ...
    async req(topic: string, ...args: Array<number | string>) {
      const id = IdCreate();
      const relArgs: Array<number | string> = [];
      if (topic === 'trade') {
        const subs: Array<number | string> = [topic];
    ...
    sub(topic: WsSubMap, ...args: Array<number | string>) {
      const id = IdCreate();
